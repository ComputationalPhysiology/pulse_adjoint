...
class ClinicalData(object):
    """
    This class is an iterable object holding the clinical
    data used in the data assimilation process.

    Data can be in parsed either as a list/tuple or as a dictionary
    whose values are list/tuples
    """
    ...
    def __init__(self,
        ...
        if self.time is None:
            self.time = range(self.num_points)
        else:
            # Make first time point zero
            self.time = np.subtract(self.time, self.time[0])
    ...
    def data_dict(self):
        ...
        for attr in ('time', 'volume', 'strain', 'pressure', 'RVV', 'RVP'):
    ...
    def from_file(cls, data_path=None, echo_path=None):
        ...
        if data_path is not None:
            ...
            # '.xls', '.xlsx', '.csv')
            msg = ("Filename has to have one of the following "
                   "extensions {}, got {}").format(extensions, ext)
    ...
    def to_yaml(self, fname):
        ...
        try:
            ...
            logger.error(("Data contains the following keys "
                          "{}").format(data.keys()))
    ...
    def load_yaml_file(fname):
        ...
        try:
            ...
            logger.error(("You need to install yaml. "
                          "pip install pyyaml"))
            raise ex
            ...
            if 'start_passive' not in data:
                data['start_passive'] \
                    = data.pop('passive_filling_begins', 0)

            if 'start_active' not in data:
                data['start_active'] \
                    = data['start_passive'] \
                    + data.pop('passive_filling_duration', 1)
