...
from pulse_adjoint.optimization_targets_new import RegionalStrainTarget, OptimizationTarget
...
def test_regional_strain_target(problem, clinical_data):

    target = RegionalStrainTarget(problem.geometry,
                                  clinical_data.strain)
    ...
    target.assign_functional()
    target.assign
    target.reset()
    target.load_target_data(target_data, 0)
...
if __name__ == "__main__":
    ...
    test_regional_strain_target(prob, data)
