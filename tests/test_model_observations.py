...
def test_strain(isochoric=True):
    ...
    y_dir = dolfin.Constant([0.0, 1.0, 0.0])
    ...
    assert abs(x_strain_obs(u).vector().get_local()[0]
               - 0.5 * ((1 + x_strain)**2 - 1)) < 1e-12
    ...
    assert abs(y_strain_obs(u).vector().get_local()[0]
               - 0.5 * ((1 + y_strain)**2 - 1)) < 1e-12


def test_boundary_observation():

    bc = pulse.NeumannBC(traction=dolfin.Constant(0.0),
                         marker=0, name='test bc')
