...
def test_volume(volumes):
    ...
    # We collect the data
    assert np.all(np.subtract(np.squeeze([v.get_local() for v in target.collector['data']]), volumes) < 1e-12)
    assert np.all(np.subtract(np.squeeze([v.get_local() for v in target.collector['model']]), model_volume) < 1e-12)
...
def test_strain(strains):
    ...
    x_dir = dolfin.Constant([1.0, 0.0, 0.0])
    V = dolfin.VectorFunctionSpace(mesh, 'CG', 2)

    # Make an incompressible deformation (with J = 1)
    x_strain = -0.1
    y_strain = 1 / (1 + x_strain) - 1
    u = dolfin.interpolate(dolfin.Expression(('x_strain * x[0]',
                                              'y_strain * x[1]',
                                              '0.0'),
                                             x_strain=x_strain,
                                             y_strain=y_strain,
                                             degree=2), V)
    strain_obs = StrainObservation(mesh=mesh,
                                   field=x_dir)

    model_strain = strain_obs(u).vector().get_local()[0]
    weight = 0.1
    target = OptimizationTarget(strains, strain_obs,
                                weight=weight, collect=False)
